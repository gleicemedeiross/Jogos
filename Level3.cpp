...
//
// Descrição:	Nível 3 do jogo Breakout
//
**********************************************************************************/
...
void Level3::Init() {
	...
	backg = new Sprite("Resources/bg3.png");
	infobar = new Sprite("Resources/infobar3.png");
	...
	Brick* brick;
	ushort totalBricks = 0;

	fin.open("L3Bricks.txt");
	fin >> value;
	while (!fin.eof())
	{
		if (fin.good()) {
	...
	fin.close();
	Game::stats.totalBricks[2] = totalBricks;
}
...
void Level3::Update()
{
	...
	else
	{
		...
		ball->invX = false;
		ball->invY = false;
		Game::stats.score[2] = player->score;
		anim3s->NextFrame();
		...
		else if (Game::stats.bricksDestroyed >= Game::stats.totalBricks[0] + Game::stats.totalBricks[1] + Game::stats.totalBricks[2]) {
			Engine::Next<EndScreen>();
		}
	}
}
...
void Level3::Draw()
{
	...
	textAux = std::to_string(player->playerSize);
	//premier->Draw(598.0f, 560.f, textAux, yellow, Layer::FRONT, 0.80f);
